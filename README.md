...
| `escapeFormulas`    | `false`                          | `boolean`                                              | Prefixes values that start with `=`, `+`, `-`, `@`, tab, or carriage return with a `'` so spreadsheet apps (Excel, Google Sheets, LibreOffice) don't interpret them as formulas. Use this when writing untrusted data to a CSV to protect against [CSV/formula injection](https://owasp.org/www-community/attacks/CSV_Injection) (CWE-1236). |
